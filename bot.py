...
#Constants to be loaded from settings
adminRank = None
maxSC = None
minSC = None
SCFile = None
...
#TODO: Check user's rank in server
def userPermission(userid):
    if userid == 205513384349794304:
        return True
    else:
        return False
...
async def SocialCredit(ctx, *args):
    ...
    #add/subtract social credit
        ...
        if not userPermission((ctx.message.author.id)):
        ...
        if(args[0] == "sub"):
            ...
            await ctx.send("Subtracting " + str(amount) + " social credit from " + user.name + "\n"
                            "New social credit is: " + str(getSocialCredit(user)))
        else:
    ...
    #Find user social credit, or save/load social credit
        ...
        await ctx.send("social credit for " + user.name + ": [" + str(userSC) + "], with [" + str(gain)  + "] gained today!")
    else:
        await badCommand(ctx)
        print(args)
